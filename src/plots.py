...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    """
    # Representar gráficamente la relación entre el valor del flete y el peso usando un scatterplot de seaborn.
    ...
    # Mejorar visualización para valores atípicos
    if df['product_weight_g'].max() > df['product_weight_g'].quantile(0.95) * 2:
        # Si hay valores atípicos extremos, ajustar la escala
        plt.xlim(0, df['product_weight_g'].quantile(0.95) * 1.5)
        plt.ylim(0, df['freight_value'].quantile(0.95) * 1.5)
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    # Convertir timestamp (milisegundos) a datetime para mejor visualización
    df = df.copy()
    ...
    # Graficar el número de órdenes por día
    ax.plot(df['date_dt'], df['order_count'], '-o', markersize=3, label='Orders')
    
    # Marcar los días festivos con líneas verticales rojas
    holiday_df = df[df['holiday'] == True]
    for _, row in holiday_df.iterrows():
        ax.axvline(x=row['date_dt'], color='red', linestyle='--', alpha=0.7)
    
    # Personalizar el gráfico
    ax.set_title('Orders per Day and Holidays in 2017')
    ax.set_xlabel('Date')
    ...
    legend_elements = [
        Line2D([0], [0], color='b', marker='o', label='Orders'),
        Line2D([0], [0], color='r', linestyle='--', label='Holidays')
    ]
    ...
    # Definir directamente las funciones en tu notebook

def my_plot_order_amount_per_day_with_holidays(df):
    import pandas as pd
    import matplotlib.pyplot as plt
    from matplotlib.lines import Line2D
    ...
    # Graficar el número de órdenes por día
    ax.plot(df['date_dt'], df['order_count'], '-o', markersize=3, color='blue')
    ...
    # Añadir leyenda
    legend_elements = [
        Line2D([0], [0], color='blue', marker='o', label='Orders'),
        Line2D([0], [0], color='red', linestyle='--', label='Holidays')
    ]
    ax.legend(handles=legend_elements)
    
    plt.show()

def my_plot_freight_value_weight_relationship(df):
    import matplotlib.pyplot as plt
    import seaborn as sns
